...
func _ready():
	level = player.tornado_level  # Obter o nível atual do Tornado do jogador
	...
	last_movement = player.last_movement  # Obter o último movimento do jogador
	...
	initial_tween.tween_property(self, "scale", Vector2.ONE * attack_size, 3).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	var final_speed = speed
	speed = speed / 5.0
	initial_tween.tween_property(self, "speed", final_speed, 6).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
	initial_tween.play()
	
	# Iniciar o movimento oscilante
	start_movement()
...
func start_movement():
	...
	# Após um tempo, destruir o tornado
	await get_tree().create_timer(12.0).timeout  # Ajuste o tempo conforme necessário
	queue_free()
